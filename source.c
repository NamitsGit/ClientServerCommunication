...
void draw(float radius)  //used to create a ellipse(circle) shaped figure in the server
{
...
void line()     //this function is to design the system(monitor and CPU)
{
	...
	//Inner blue part of the monitor
	glColor3f(0.535, 0.808, 0.937);
	glBegin(GL_POLYGON);
	...
	//CPU POWER BUTTON SMALL RECTANGLE
	glColor3f(0.0, 0.0, 0.0);
	glBegin(GL_POLYGON);
	...
	//2 3.5mm jacks on CPU ON LEFT SIDE
	glColor3f(0.0, 0.0, 0.0);
	glPointSize(3.5);
	glBegin(GL_POINTS);
	...
	//BLACK BOX CONTAINING RED AND BLUE DOT IN CPU
	glColor3f(0.0, 0.0, 0.0);
	glBegin(GL_POLYGON);
...
void server() // this function is to design the central server.
{
	...
	//YELLOW LINE NEAR THE SERVER CPU POWER BUTTON
...
void upload4() // used to display the bitmap characters for client 3's uploaded text
{
	glPushMatrix();
...
void uploading4()// used to display the bitmap characters for client 4's uploading text
{
	glPushMatrix();
	glTranslatef(100.0, 0.0, 0.0);
...
void download4()// used to display the bitmap characters for Client 4's downloaded text
{
...
void downloading4()// used to display the bitmap characters for Client 4's downloading text
{
	glPushMatrix();
	glTranslatef(100.0, 0.0, 0.0);
...
void download3()// used to display the bitmap characters for Client 3's downloaded text
{
	glPushMatrix();
	glTranslatef(100.0, 0.0, 0.0);
...
void request()// used to display the bitmap characters.
{
	glPushMatrix();
	glTranslatef(100.0, 0.0, 0.0);
	ptr3 = "H A N D S H A K E   P R O C E S S - R E Q U E S T";
...
void response()// used to display the bitmap characters.
{
	glPushMatrix();
	glTranslatef(100.0, 0.0, 0.0);
	ptr4 = "H A N D S H A K E   P R O C E S S - R E S P O N S E";

	len2 = strlen(ptr4);

	//glColor3f(0.60, 0.60, 0.60);
	glRasterPos3f(-10.0, 190.0, 0.0);
...
void display()//this function displays all the design and simulations.
{
	...
	glRasterPos3f(-53, 20, 0.0);   // This position, called the raster position, is used to position pixel and bitmap write operations. 
								   //It is maintained with subpixel accuracy.
	for (int i = 0; i < len5; i++)
